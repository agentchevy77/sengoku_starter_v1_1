...
def gen_alerts(snapshot: Dict[str, Any], thresholds: Dict[str, int] | None = None) -> List[Dict[str, Any]]:
    ...
    if score >= th["score_attack"]:
        out.append({
            "symbol": symbol, "kind": "score_attack", "value": score,
            "threshold": th["score_attack"], "severity": _sev(score, th["score_attack"]),
    ...
    if score <= th["score_defend"]:
        ...
        out.append({
            "symbol": symbol, "kind": "score_defend", "value": score,
            "threshold": th["score_defend"], "severity": sev,
    ...
    for k in keys:
        ...
        if v >= t:
            out.append({
                "symbol": symbol, "kind": k, "value": v,
                "threshold": t, "severity": _sev(v, t),
