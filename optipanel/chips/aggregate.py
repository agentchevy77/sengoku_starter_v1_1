...
try:  # Stage 3 metrics: optional instrumentation hooks
    from optipanel.obs.metrics import record, timer
except Exception:  # pragma: no cover - metrics module optional during tests
...
def aggregate_chips(
    ...
    record("chips.aggregate.calls")
    with timer("chips.aggregate.ms"):
        if not chips_by_tf:
            return {}
...
def recon_score(agg: dict[str, int]) -> int:
    ...
    record("recon.score.calls")
    with timer("recon.score.ms"):
