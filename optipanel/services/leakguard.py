...
class LeakSentinel:
    ...
    def __enter__(self):
        tracemalloc.start()
        self._snap_a = tracemalloc.take_snapshot()
        return self
        ...
        for c in list(self._closers):
            try:
                ...
                if hasattr(c, "aclose"):
                    meth = c.aclose
                    await meth()
                elif hasattr(c, "close") and asyncio.iscoroutinefunction(c.close):
                    meth = c.close
                    await meth()
                elif hasattr(c, "shutdown") and asyncio.iscoroutinefunction(c.shutdown):
                    meth = c.shutdown
                    await meth()
                else:
                    # try synchronous fallbacks
                    if hasattr(c, "close"):
                        c.close()
                    elif hasattr(c, "shutdown"):
                        c.shutdown()
            except Exception:
                pass
        self._tasks.clear()
...
class PeriodicTask:
    ...
    def start(self):
        ...
        self.registry.track_task(self._task)
        ...
        try:
            while not self._stop.is_set():
                try:
                    ...
                    self._failures += 1
                    logger.exception(
                        "PeriodicTask %s failure #%d; continuing",
                        self.name,
                        self._failures,
                    )
