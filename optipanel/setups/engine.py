...
def compute_setups(features: dict[str, Any]) -> dict[str, int]:
    ...
    if gap <= 0:
        ...
        base = 60.0 + 25.0 * (1 - gap / 0.01)  # 60..85 closer
    else:
        base = 30.0
    ...
    if last < support:
        ...
        base = 60.0 + 25.0 * (1 - max(0.0, near_sup) / 0.01)
    else:
        base = 30.0
    bonus = (10 if rvol >= 1.2 else 0) + (10 if rs <= -0.1 else 0) + (5 if last < dma20 else 0)
    ...
    if last >= support:
        ...
        base = 50.0 + (20.0 * (1 - near / 0.01)) if near <= 0.01 else 35.0
    ...
    if last <= resistance:
        ...
        base = 45.0 + (15.0 * (1 - near / 0.01)) if near <= 0.01 else 30.0
        malus = 0
        if last < dma20:
            malus += 10
        if rs <= 0.0:
    ...
    exh = 30.0
    if ext >= 0.05:
        exh = 60.0 + 20.0 * min(1.0, (ext - 0.05) / 0.05)  # 5–10% -> 60–80
    if rvol >= 1.5:
        exh += 10
